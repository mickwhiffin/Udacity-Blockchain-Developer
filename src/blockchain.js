...
class Blockchain {
    ...
    _addBlock(block) {
        ...
        return new Promise(async (resolve, reject) => {
            ...
            let chainIsValid = await self.validateChain();
    ...
    requestMessageOwnershipVerification(address) {
        return new Promise((resolve) => {
            ...
            resolve(`${address}:${date}:starRegistry`);
        });
    }
        ...
        return new Promise(async (resolve, reject) => {
            ...
            if ((currentTime-time) <= 300) {
                ...
                if(isVerified) {
                    ...
                    reject('Signature could not be verified');
                }
            } else {
     ...
     * 2. Each Block should be checked against the previousBlockHash
     */
    validateChain() {
        let self = this;
        let errorLog = [];
            ...
            self.chain.forEach(block => {
                ...
                if(block.height > 0) {
                    ...
                    if(blockHash != previousBlockHash){
                        errorLog.push(`Error - Block Height is ${block.height} - The Previous Hash does't match.`);
                    }
                }
                chainIndex++;
            });
            Promise
                // Need to check if there is a better way of doing this. 
                // Had issues with Promise.all in the past
                .all(promises)
                .then((results) => {
                    chainIndex = 0;
                    results.forEach(resultIsValid => {
                    if(!resultIsValid){
